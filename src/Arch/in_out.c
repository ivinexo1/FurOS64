...
static inline void outd(uint16_t port, uint32_t val) {
  __asm__ volatile("outd %d0, %w1" : : "a"(val), "Nd"(port) : "memory");
  /* There's an outb %al, $imm8 encoding, for compile-time constant port numbers
   * that fit in 8b. (N constraint). Wider immediate constants would be
   * truncated at assemble-time (e.g. "i" constraint). The  outb  %al, %dx
   * encoding is the only option for all other cases. %1 expands to %dx because
...
static inline uint32_t ind(uint16_t port) {
  uint32_t ret;
  __asm__ volatile("ind %w1, %d0" : "=a"(ret) : "Nd"(port) : "memory");
  return ret;
}

static inline uint64_t inl(uint16_t port) {
  uint64_t ret;
  __asm__ volatile("ind %w1, %l0" : "=a"(ret) : "Nd"(port) : "memory");
  return ret;
}
